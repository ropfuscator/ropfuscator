...
You can use `ropfuscator:llvm-7` image. This image provides `clang-7` and `llc` commands, which run ROPfuscator.
...
- clang (the same version as used in building ROPfuscator; clang-7 by default)
