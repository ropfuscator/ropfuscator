...
- Opaque Predicates
    ...
    - Supported opaque constant algorithms: Integer factorization-based, Random 3SAT-based, dummy
        - Integer factorization-based (`multcomp`) [recommended]: concatenate 32 outputs from opaque predicates. Each opaque predicate takes 2 (32-bit) input values and multiply them, and compare the generated (64-bit) value to a 64-bit prime value. The opaque predicate returns always 0 (to get 1, negate the result).
        - Random 3-SAT based (`r3sat32`): concatenate 32 outputs from opaque predicates. Each opaque predicate takes 32bit value, and interpret each bit as boolean variable, and test the result of a 3CNF formula. The 3CNF formula is large enough and thus unsatisfiable with a probability nearly equal to 1. This means that the opaque predicate returns almost always 0. Note that the opaque predicate may generate an incorrect output in very small possibility (when the clause is satisfiable, and the random value is the satisfiable input); if this is not acceptable, this opaque predicate should not be used.
        - Dummy algorithm (`mov`): This is not actually an "opaque" predicate - it just outputs the constant using `mov` instruction. This is not meant for production use, but only for testing opaque constant interface.
    - Input generation strategy: there are several strategies to generate inputs to opaque predicates. These inputs are meant to be random or not predictable statically, to hinder program analysis techniques such as DSE (DSE is expected to treat these inputs as "symbolic" values, which causes diverge in exploration). Algorithms include:
    ...
    - **Mixture of invariant and contextual opaque predicates**: use contextual opaque predicate (output changes based on input values) to avoid opaque predicate identification attack by pattern matching
        ...
        - Integer factorization-based (`multcomp`): An invariant opaque predicate takes two 32-bit random inputs and compares the multiplication result to constant 64-bit prime number, and always generates 0. On the other hand, a contextual opaque predicate compares the multiplication of two input values with the multiplication of two 32-bit prime numbers; it generates 1 for those 2 crafted values, and generates 0 otherwise. This kind of contextual opaque predicates are used in conjunction with invariant opaque predicates to generate an opaque constant.
...
- Opaque predicates - **branch divergence**
    ...
    - It is typical that there are several equivalent ROP gadgets in the same binary (e.g., there can be multiple `pop edx; ret` gadgets, and there can be `push eax; ret` or `jmp eax` which are almost equivalent). Normally, ROPfuscator statically choose one of the equivalent gadgets randomly and push the gadget address. When the branch divergence is used, ROPfuscator dynamically choose one of the equivalent gadgets randomly. The execution path will diverge by choosing from multiple gadgets (at different addresses) at runtime. This makes some analysis with execution path tracking (such as DSE) infeasible to trace paths.
    - Formally speaking, branch divergence computes `y = f(x)`, where `y` is the gadget address and `x` is a run-time random value. `f` is crafted such that the `y` falls into specific set of values (possible gadget addresses) based on `x`, whatever the value of `x` is. For example, if possible gadget addresses are `0x8040123`, `0x8080456`, `0x8040789`, then `f` should generate either one of these values from arbitrary 32-bit value.
    - In the current implementation, the above `f` is implemented as a composition of the following steps:
        - (1) Random input generation: generate random `x` at runtime.
        - (2) Selection of random constants based on random input: `r = selector(x, random constants)` where `random constants` is determined at compile time, and `selector` randomly selects one of possible values of `random constants` depending on `x` at runtime.
    ...
    - Algorithm configuration
        - The (1) input value generation algorithm and the (2) selection algorithm can be configured by `branch_divergence_algorithm` option.
    ...
    - (3) Adjusting algorithm
        ...
        - If we cannot find an inverse matrix in the end, then we use divide-and-conquer strategy: split the vector `x`, `y` in smaller vectors and repeat the above algorithm.
...
Obfuscation algorithms can be switched on/off in a per-function basis. Configuration file provides a number of options to control the algorithms for each function.
...
- ROP Transformation
    ...
    - Gadget extraction options
        ...
        - To avoid library version confusion, library SHA1 hash can be specified with `library_hash_sha1` option.
        - Gadget search is done in either ELF executable segments or ELF code sections. Generally, we can find more gadgets in executable segments, since segments include broader memory region than code sections (e.g., read-only data can be put into executable segments). This behavior can be controlled by `search_segment_for_gadget` option (true: use segments, false: use sections)
...
- Opaque predicates
    - Can be switched on/off by `opaque_predicates_enabled` option (Note: if opaque predicate is off, branch divergence and instruction hiding cannot be turned on)
    ...
    - Mangling stack saved values: This feature stores register values and statically generated random constants onto stack in a random order, and use those stack values in opaque predicate computation, making it harder for attackers to identify what is going on in opaque predicates. This can be turned on/off using `obfuscate_stack_saved_values` option.
- Opaque predicates - branch divergence
