/*
 * C Program to Create Employee File Name Record that is taken from the
...
void insert(char *a) {
  ...
  temp1->name =
      (char *)malloc(200 * sizeof(char)); // allocating memory for pointer
...
void update(char *a) {
  ...
  if (fp1 == NULL)
    ...
    while (var) // displaying employee records so that user enter correct
                // employee id
    {
      fread(&temp->empid, sizeof(int), 1, fp1);
      printf("%d", temp->empid);
